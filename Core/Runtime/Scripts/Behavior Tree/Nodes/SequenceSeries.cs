...
namespace Lord.Core {
    public class SequenceSeries : Node {
        ...
        public SequenceSeries(List<Node> nodes) {
            this.nodeList = nodes;
            currentIndex = 0;
            previousIndex = 0;
            iterations = 0;
        ...
        public override NodeStates Evaluate() {
            ...
            while (_runningIteration) {
                ...
                if (currentIndex == previousIndex) { // sequence has not progressed
                    Debug.Log("[BT SEQUENCE SERIES] Sequence (" + (currentIndex + 1) + "/" + nodeList.Count + ")" + " " + _node.nodeState + "-" + _node.nodeName);
                    _runningIteration = false;
                } else { // sequence has progressed
                    previousIndex = currentIndex;
                    Debug.Log("[BT SEQUENCE SERIES] Sequence (" + currentIndex + "/" + nodeList.Count + ")" + " " + _node.nodeState + "-" + _node.nodeName);
                ...
                if (iterations >= 100) {
                    _runningIteration = false;
                    Debug.LogWarning("[BT SEQUENCE SERIES] INFINITE LOOP POSSIBLY DETECTED");
                }
                iterations += 1;
            }
            return nodeState;
