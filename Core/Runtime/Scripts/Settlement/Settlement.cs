...
namespace Lord.Core{
    ...
    public class Settlement
    {
            ...
            for (int i = 0; i < 10; i++){
                ...
                while(_searching && _count < 10){
                    ...
                    if(IsValidBuilding(_buildingPosition, _buildingSize, _direction)){
                        GenerateBuilding(_buildingPosition, _buildingSize, _direction);
                        _searching = false;
                    }
                    _count++;
                }
        ...
        private void GenerateBuilding(Vector2Int startPos, Vector2Int size, Direction direction=Direction.NORTH){
            for (int i = 0; i < size.x; i++){
                for(int j = 0; j < size.y; j++){
                    Vector2Int _offset = new Vector2Int(i,j);
                    Vector2Int _currentPosition = GenerateBuildingPos(startPos, _offset, direction);
                    // switch (direction){
                    //     case Direction.NORTH:
                    //         _currentPosition = new Vector2Int(startPos.x + _offset.x, startPos.y + _offset.y);
                    //         break;
                    //     case Direction.EAST:
                    //         _currentPosition = new Vector2Int(startPos.x + _offset.x, startPos.y + _offset.y);
                    //         break;
        ...
        private Vector2Int GenerateBuildingPos(Vector2Int startPos, Vector2Int offset, Direction direction){
        ...
        private void GenerateRoads(){
            // List<Vector2Int> _roads = new List<Vector2Int>();
            // _roads.Add(new Vector2Int(0,0));
            for(int i=0;i<4;i++){
                Debug.Log("DIRECTION COUNT " + i + (Direction)i);
                GenerateRoads(new Vector2Int(0,0), settlementRadius, (Direction)i);
            }
        }
        private void GenerateRoads(Vector2Int startPos, int length, Direction direction, float spawnPercent=0.1f){
            for (int i = 0; i < length; i++){
                ...
                if(Settlement.Rand.NextDouble() < spawnPercent){
                    ...
                    GenerateRoads(_currentPosition, length/2, _direction, spawnPercent=0.0f);
                }
            }
        }
        private bool IsValidBuilding(Vector2Int startPos, Vector2Int size, Direction direction){
            ...
            for (int i = -1; i < size.x + 1; i++){
                for(int j = -1; j < size.y + 1; j++){
                    Vector2Int _currentPosition = GenerateBuildingPos(startPos, new Vector2Int(i,j), direction);
                    bool _exists;
